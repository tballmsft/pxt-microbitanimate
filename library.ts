    ...
    export class AnimationQueue {
        ...
        runUntilDone(render: () => boolean) {
            ...
            if (this.running) {
                ...
                //}
                //control.waitForEvent(DAL.MICROBIT_ID_NOTIFY, evid);
            }
            ...
            this.running = false;
            // unblock 1 fiber
            control.raiseEvent(DAL.MICROBIT_ID_NOTIFY, this.eventID);
        }
        ...
        cancel() {
            if (this.running) {
                ...
                this.eventID = control.allocateNotifyEvent();
                // unblock fibers
                control.raiseEvent(DAL.MICROBIT_ID_NOTIFY, evid);
            }
        }
    }
}
...
namespace light {
    ...
    export class NeoPixelStrip {
         ...
         */
        get buf(): Buffer {
            return this._microbitStrip.buf;
            /*
        ...
        setPixelWhiteLED(pixeloffset: number, white: number): void {
            if (this._mode != NeoMode.RGBW) return;
        ...
        show(): void {
            ...
            if (this._parent) this._parent.show();
                ...
                const stride = this.stride();
                const strideOffset = this._mode == NeoMode.APA102 ? 1 : 0;
                // apply brightness
                ...
                for (let i = 0; i < b.length; i++) {
                    this._microbitStrip.buf[i] = sb[i]
                }
                this._microbitStrip.show()
            ...
            if (this._photonPenColor) {
                ...
                for (let bi = 0; bi < tailn && c > 0; ++bi) {
                    if (this._mode == NeoMode.RGBW)
                        sb[pi + 3] = c;
                    else if (this._mode == NeoMode.APA102)
        ...
        */
        ...
        private stride(): number {
            return this._mode === NeoMode.RGBW || this._mode == NeoMode.APA102 ? 4 : 3;
        }
